        ...
        if (value) {
          ...
          $inputs.each((int, elem) => {
            if ($(elem).prop('checked')) {
              values['blackgrass-ranges'] = $(elem).val();
            }
          });
        }
        break;
  ...
  serializeData() {
    ...
    switch (this.options.attr) {
      case 'date':
        ...
        break;
      case 'blackgrass':
        let blackgrass = occ.get('blackgrass');
        if (blackgrass) {
          templateData.blackgrass = blackgrass;
          templateData.blackgrassPosition = logsl.position(blackgrass).toFixed(0);
        } else {
          blackgrass = occ.get('blackgrass-ranges') || 'default';
          templateData[blackgrass] = true;
        }
        break;
      case 'season':
        templateData[occ.get('season')] = true;
        break;
      case 'comment':
